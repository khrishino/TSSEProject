...
//	private final float[] wFactor = {0.26f, 0.45f, 0.73f, 1.0f};
	...
	/* L'utilit� di tale struttura dati � spiegata all'inizio del metodo "CreateStaticThings" */
	ArrayList<Rectangle> rect;
	
	/* Traiettorie � una Mappa che ha come chiave l'id di una persona (Stringa), e come valore
	...
	/* Costruttore */
	public OntPopulation(VirtGraph graph, String NS, String trackingOutputFile, int groupId, ArrayList<String> groups, HashMap<Integer, String> lastPersonDirection, HashMap<Integer, Integer> samePersonDirectionFrames, HashMap<Integer, Integer> directionChanges, HashMap<String, ArrayList<ArrayList<Integer>>> Traiettorie){
		this.graph = graph;
		this.NS = NS;
	...
	public void createPassingArea(Point bottomRight, Point topLeft){
    	...
    	/* Definisco un rettangolo che tornerà utile per il controllo dell'intersezione di un BoundingBox con una specifia area*/
	...
	public void createSinglePerspectiveArea(int id, Point tLV, Point bRV, float wF, float hF, int minH, int minW, int maxH, int maxW){
		VirtuosoUpdateRequest vur;
		...
		// Creo l'individuo perspective area e gli setto le propriet�
        ...
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"PerspectiveArea"+id+"> <"+NS+"minHumanHeight> '"+minH+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
		
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"PerspectiveArea"+id+"> <"+NS+"minHumanWidth> '"+minW+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
        
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"PerspectiveArea"+id+"> <"+NS+"maxHumanHeight> '"+maxH+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
        
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"PerspectiveArea"+id+"> <"+NS+"maxHumanWidth> '"+maxW+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
	...
	public void createPerspectiveAreas(){
		...
		createSinglePerspectiveArea(1, new Point(0,0), new Point(768,100), hFactor[0], wFactor[0], minHPerspArea[0], minWPerspArea[0], maxHPerspArea[0], maxWPerspArea[0]);
		
		// Prima perspective area da y=100 a y=240
		createSinglePerspectiveArea(2, new Point(0,101), new Point(768,240), hFactor[1], wFactor[1], minHPerspArea[1], minWPerspArea[1], maxHPerspArea[1], maxWPerspArea[1]);
				
		// Prima perspective area da y=240 a y=380
		createSinglePerspectiveArea(3, new Point(0,241), new Point(768,380), hFactor[2], wFactor[2], minHPerspArea[2], minWPerspArea[2], maxHPerspArea[2], maxWPerspArea[2]);
				
		// Prima perspective area da y=380 a y=576
		createSinglePerspectiveArea(4, new Point(0,381), new Point(768,576), hFactor[3], wFactor[3], minHPerspArea[3], minWPerspArea[3], maxHPerspArea[3], maxWPerspArea[3]);
	}
	...
	public String createBlob(String individual_id, String[] temp2 /* id BoundingBox + coordinate vertici TL e BR*/, String idFrame){
		Triple t = null;
		...
		str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"CenterPointBlob"+individual_id+"> <"+NS+"x> '"+center.x+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
        
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"CenterPointBlob"+individual_id+"> <"+NS+"y> '"+center.y+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
    	...
    	// Flag che dice se il bounding box ha dimensioni appropriate per la perspective area a cui appartiene
    	...
    	/* Se è la prima volta che appare questa persona:
    	 * - avrà la proprietà "firstSeenAt"
    	 * - non ha direzione (viene calcolata rispetto al frame precedente), e quindi verrà settata "hasDirection" a "stopped"
    	 * - lo stesso per la proprietà "hasSpeed" messa a "0"
    	 */
    	...
    	if(!graph.contains(new Triple(s9, p9, o9))) { 
    		if (lowerLimitRespect){
    			...
    			str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"Person"+temp2[0]+"> <"+NS+"id> '"+temp2[0]+"'}";
    			vur = VirtuosoUpdateFactory.create(str, graph);
    	        vur.exec();
	    		...
	    		// popolo le hash map per la gestione dei cambi di direzione / frame di stessa direzione
	    		...
	    		str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"Blob"+individual_id+"> <"+NS+"sameDirectionFrames> '1'}";
    			vur = VirtuosoUpdateFactory.create(str, graph);
    	        vur.exec();
	    		
	    		this.directionChanges.put(Integer.parseInt(temp2[0]), 0);
	    		
	    		str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"Blob"+individual_id+"> <"+NS+"directionChanges> '0'}";
    			vur = VirtuosoUpdateFactory.create(str, graph);
    	        vur.exec();
	        	...
	        	/* Dato che è la prima volta che appare tale persona inizializzo l'ArrayList relativa alla sua Traiettoria e 
	        	 * vi aggiungo il primo punto */
	        	traiettoria_i = new ArrayList<>();
				traiettoria_i.add(punto_i);
				/* MANTENGO TRACCIA DELLA TRAIETTORIA ASSOCIANDO QUEST'ULTIMA ALL'ID DELLA PERSONA*/
        	...
        	str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"Blob"+individual_id+"> <"+NS+"sameDirectionFrames> '"+samePersonDirectionFrames.get(Integer.parseInt(temp2[0]))+"'}";
			vur = VirtuosoUpdateFactory.create(str, graph);
	        vur.exec();
	        
	        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"Blob"+individual_id+"> <"+NS+"directionChanges> '"+directionChanges.get(Integer.parseInt(temp2[0]))+"'}";
			vur = VirtuosoUpdateFactory.create(str, graph);
	        vur.exec();
        	...
        	if(!lastPersonDirection.get(Integer.parseInt(temp2[0])).equals(direction)){
    			if(newPersonDirection.get(Integer.parseInt(temp2[0]))==null) {
    				...
    				newPersonDirection.put(Integer.parseInt(temp2[0]),newPersonDirection.get(Integer.parseInt(temp2[0]))+1);
    			}
    			if (newPersonDirection.get(Integer.parseInt(temp2[0]))>7) {
        			...
        			directionChanges.put(Integer.parseInt(temp2[0]), directionChanges.get(Integer.parseInt(temp2[0]))+1);
    	...
    	str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"Blob"+individual_id+"> <"+NS+"blobid> '"+individual_id+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
    	...
    	if (!lowerLimitRespect){
    		str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"Blob"+individual_id+"> <"+NS+"malformedBlob> 'true'}";
    		vur = VirtuosoUpdateFactory.create(str, graph);
            vur.exec();
    	}
    	...
    	if (!personIgnored) {
    		...
    		str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+direction+"> <http://www.w3.org/2000/01/rdf-schema#label> '"+direction+"'}";
    		vur = VirtuosoUpdateFactory.create(str, graph);
            vur.exec();
	    	...
	    	/* Setto la Velocità*/
    		str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"Blob"+individual_id+"> <"+NS+"hasSpeed> '"+String.valueOf(speed)+"'}";
    		vur = VirtuosoUpdateFactory.create(str, graph);
            vur.exec();
	    	
	    	/* Setto il tempo*/
            str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"Blob"+individual_id+"> <"+NS+"timeOnScene> '"+String.valueOf(time)+"'}";
    		vur = VirtuosoUpdateFactory.create(str, graph);
            vur.exec();
    	}
	    ...
	    if (!upperLimitRespect) {
	    	if(trackingOutputFile.equals("src//view1.txt")){
	    		str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"Person"+temp2[0]+"> <"+NS+"isProbablyAGroup> 'true'}";
	    		vur = VirtuosoUpdateFactory.create(str, graph);
	            vur.exec();
	    	}
	    }
    	
    	/* Ora si procede al calcolo dell'area in cui si trova il BoundingBox relativo alla persona i-esima (Entry,Exit,Occluding)*/
		/* indice 0 = entryArea1: in basso a destra;
		 * indice 1 = entryArea2: in alto un po' a sinistra
		 * indice 2 = entryArea3: in alto un po' a destra
		 * indice 3 = exitArea1: sopra a sinistra
		 * indice 4 = exitArea2: un po' pi� in basso a sinistra 
		 * indice 5 = occludingArea1: il palo compreso il cartello attaccato
	...
	public void createGroups(ArrayList<String> peopleOfAFrame, String frame){
		...
		String build, direction1 = null, direction2 = null, bRV_x_st, bLV_x_st, bLV_y_st, frameId_st, currentMember, last_seen_frame_sub = null, first_seen_frame_sub = null, dir2 = null, bRV = null, bLV = null, dir1 = null, last_seen_frame_st = null, first_seen_frame_st = null;
		int x1, y1, x2 = 0, y2 = 0;
		String currentGroup, str;
		boolean sameDirection, existingGroup, containFirstPerson, containSecondPerson, i1_isProbablyAGroup = false, i2_isProbablyAGroup = false;
		...
		RDFNode bRV_x = null, bLV_x = null, bLV_y = null, direct1 = null, direct2 = null, Node_frameId = null, Node_last_seen = null, Node_first_seen = null;
		ResultSet results;
		...
		for (String individ1 : peopleOfAFrame){			// Ciclo sulle persone
			for (String individ2 : peopleOfAFrame){		// Ciclo sulle persone
				...
				if (individ1!=null && individ2!=null && !individ1.equals(individ2)){
					...
					build = "SELECT * FROM <"+GRAPH+"> WHERE { <"+dir1+"> <http://www.w3.org/2000/01/rdf-schema#label> ?o }";
...
//					System.out.println(blob.getLocalName());
					...
					build = "SELECT * FROM <"+GRAPH+"> WHERE { <"+dir2+"> <http://www.w3.org/2000/01/rdf-schema#label> ?o }";
					vqe = VirtuosoQueryExecutionFactory.create (build, graph);
					...
					// Calcolo le opportune soglie come media delle soglie delle rispettive perspective
					// (NB: se le perspective sono le stesse si ottengono le soglie proprie di quella perspective)
					int threshX = Math.round((xGropuThreshold * wFactor[perspective1]+
											  xGropuThreshold * wFactor[perspective2])/2);
					int threshY = Math.round((yGropuThreshold * hFactor[perspective1]+
											  yGropuThreshold * hFactor[perspective2])/2);
					...
					if(frameId>64 && frameId<89 &&
							((individ1.equals("Person1") && individ2.equals("Person4")) ||
...
//						System.out.println("direzioni "+direction1+" "+direction2);
					...
					if(sameDirection && (Math.abs(x1-x2)<=threshX)&&(Math.abs(y1-y2)<=threshY)) { // Da considerare gruppo
						existingGroup = false;
						...
						while(groupIt.hasNext() && !existingGroup) {
							...
							// Controllo se il generico gruppo esistente contiene gi� le due persone in esame
							...
							// Se il gruppo non � pi� vecchio di un certo numero di frame
							...
							build = "SELECT * FROM <"+GRAPH+"> WHERE { <"+last_seen_frame_sub+"> <"+NS+"id> ?o }";
							vqe = VirtuosoQueryExecutionFactory.create (build, graph);
							...
							// Se le contiene allora aggiorno solo la propriet� "lastSeenAt"
							if (groupOldness<=oldGroupThreshold && containFirstPerson && containSecondPerson) {
								...
								build = "SELECT * FROM <"+GRAPH+"> WHERE { <"+first_seen_frame_sub+"> <"+NS+"id> ?o }";
								vqe = VirtuosoQueryExecutionFactory.create (build, graph);
					    		...
					    		build = "SELECT * FROM <"+GRAPH+"> WHERE { <http://xmlns.com/foaf/0.1/"+currentGroup+"> <"+NS+"groupSince> ?o }";
								vqe = VirtuosoQueryExecutionFactory.create(build, graph);
								results = vqe.execSelect();
								while (results.hasNext()) {
									QuerySolution res = results.nextSolution();
								    String obj = res.get("o").asLiteral().toString();
								    str = "DELETE FROM GRAPH <"+GRAPH+"> { <http://xmlns.com/foaf/0.1/"+currentGroup+"> <"+NS+"groupSince> '"+obj+"' }";
					                vur = VirtuosoUpdateFactory.create(str, graph);
					                vur.exec();
								}
								
				    			str = "INSERT INTO GRAPH <"+GRAPH+"> { <http://xmlns.com/foaf/0.1/"+currentGroup+"> <"+NS+"groupSince> '"+groupSince+"'}";
					    		vur = VirtuosoUpdateFactory.create(str, graph);
					            vur.exec();
								
								existingGroup = true;
//								System.out.println("Modificato il gruppo "+currentGroup.getLocalName()+" con le persone "+individ1.getLocalName()+" "+individ2.getLocalName());
							}
						}
...
//							System.out.println("Creato il gruppo "+newGroup.getLocalName()+" con le persone "+individ1.getLocalName()+" "+individ2.getLocalName());
							...
							str = "INSERT INTO GRAPH <"+GRAPH+"> { <http://xmlns.com/foaf/0.1/Group"+groupId+"> <"+NS+"id> '"+groupId+"'}";
				    		vur = VirtuosoUpdateFactory.create(str, graph);
				            vur.exec();
							...
							build = "SELECT * FROM <"+GRAPH+"> WHERE { <http://xmlns.com/foaf/0.1/Group"+groupId+"> <"+NS+"groupSince> ?o }";
							vqe = VirtuosoQueryExecutionFactory.create (build, graph);
							results = vqe.execSelect();
							while (results.hasNext()) {
								QuerySolution res = results.nextSolution();
							    String obj = res.get("o").asLiteral().toString();
							    str = "DELETE FROM GRAPH <"+GRAPH+"> { <http://xmlns.com/foaf/0.1/Group"+groupId+"> <"+NS+"groupSince> '"+obj+"' }";
				                vur = VirtuosoUpdateFactory.create(str, graph);
				                vur.exec();
							}
							
			    			str = "INSERT INTO GRAPH <"+GRAPH+"> { <http://xmlns.com/foaf/0.1/Group"+groupId+"> <"+NS+"groupSince> '"+groupSince+"'}";
				    		vur = VirtuosoUpdateFactory.create(str, graph);
				            vur.exec();
							
							groups.add("Group" + groupId);
							
							// Se una delle due persone era probabilmente un gruppo allora viene settato che il gruppo
							// � tale dall'entrata in scena di entrambe le persone, ovvero non � nato da un incontro
							boolean flag1 = false;
							build = "SELECT * FROM <"+GRAPH+"> WHERE { <"+NS+individ1+"> <"+NS+"isProbablyAGroup> ?o }";
							vqe = VirtuosoQueryExecutionFactory.create (build, graph);
							results = vqe.execSelect();
							while (results.hasNext()) {
								QuerySolution rs = results.nextSolution();
							...
							boolean flag2 = false;
							build = "SELECT * FROM <"+GRAPH+"> WHERE { <"+NS+individ2+"> <"+NS+"isProbablyAGroup> ?o }";
							vqe = VirtuosoQueryExecutionFactory.create (build, graph);
							results = vqe.execSelect();
							while (results.hasNext()) {
								QuerySolution rs = results.nextSolution();
							...
							if ((flag1 && i1_isProbablyAGroup) || (flag2 && i2_isProbablyAGroup)) {
								str = "INSERT INTO GRAPH <"+GRAPH+"> { <http://xmlns.com/foaf/0.1/Group"+groupId+"> <"+NS+"groupSinceEntry> 'true'}";
					    		vur = VirtuosoUpdateFactory.create(str, graph);
					            vur.exec();
							} else {
								str = "INSERT INTO GRAPH <"+GRAPH+"> { <http://xmlns.com/foaf/0.1/Group"+groupId+"> <"+NS+"groupSinceEntry> 'false'}";
					    		vur = VirtuosoUpdateFactory.create(str, graph);
					            vur.exec();
							}
							groupId++;
	...
	/* Anche in questo caso, definisco prima i vertici dei boundingBoxes, poi i boundingBoxes e poi le Aree e gli occludingObjects*/
	
	/* NOTA IMPORTANTE: tale metodo ritorna un Array di rettangoli che rappresentano le aree relative alle Entry,Exit e Occluding Area;
	 * ci� viene fatto siccome in questo modo abbiamo possibilit� di conoscere facilmente se un punto (il centro del BoundingBox 
	 * relativo alla perosna i-esima) � presente all'interno di tali Aree.*/
	public ArrayList<Rectangle> createStaticThings(){
    	...
    	/* Definisco un rettangolo che torner� utile per il controllo dell'intersezione di un BoundingBox con una specifia area*/
    	int width = topRight.x - topLeft.x;
    	int height = bottomLeft.y - topLeft.y;
		...
		/*Seconda ExitArea (un po' pi� in basso a sinistra)*/
		...
		/*Terzo Occluding Object (l'oggetto sulla strada in alto un po' a sinistra [equivale alla OArea di prima])*/
	...
	public void setBoundingBox(String ind, String individual_id, Point topLeft, Point topRight, Point bottomRight, Point bottomLeft){
		VirtuosoUpdateRequest vur;
		String str;
		...
		str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"TopLeftPoint"+individual_id+"> <"+NS+"x> '"+topLeft.x+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
		
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"TopLeftPoint"+individual_id+"> <"+NS+"y> '"+topLeft.y+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
        
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"TopRightPoint"+individual_id+"> <"+NS+"x> '"+topRight.x+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
		
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"TopRightPoint"+individual_id+"> <"+NS+"y> '"+topRight.y+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
		
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"BottomLeftPoint"+individual_id+"> <"+NS+"x> '"+bottomLeft.x+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
		
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"BottomLeftPoint"+individual_id+"> <"+NS+"y> '"+bottomLeft.y+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
        
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"BottomRightPoint"+individual_id+"> <"+NS+"x> '"+bottomRight.x+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
		
        str = "INSERT INTO GRAPH <"+GRAPH+"> { <"+NS+"BottomRightPoint"+individual_id+"> <"+NS+"y> '"+bottomRight.y+"'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
        vur.exec();
	...
	public void saveParameters(int idBlob) {
		...
		param.add(this.Traiettorie);
	      ...
	      try {
	         ...
	         new FileOutputStream("src/saved_parameters.ser");
