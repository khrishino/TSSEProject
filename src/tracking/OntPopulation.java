...
public class OntPopulation {
	...
	/*
	 * L'utilit� di tale struttura dati � spiegata all'inizio del metodo
	 * "CreateStaticThings"
	 */
	ArrayList<Rectangle> rect;

	/*
	 * Traiettorie � una Mappa che ha come chiave l'id di una persona (Stringa), e
	...
	public String createBlob(String individual_id, String[] temp2 /* id BoundingBox + coordinate vertici TL e BR */,
		...
		str = "INSERT INTO GRAPH <" + GRAPH + "> { <" + NS + "CenterPointBlob" + individual_id + "> <" + NS + "y> '"
				+ center.y + "'}";
		vur = VirtuosoUpdateFactory.create(str, graph);
		vur.exec();
		...
		if (!graph.contains(new Triple(s9, p9, o9))) {
			if (lowerLimitRespect) {
				...
				/*
				 * Dato che è la prima volta che appare tale persona inizializzo l'ArrayList
		 ...
		 * exitArea1: sopra a sinistra indice 4 = exitArea2: un po' pi� in basso a
				...
				if (individ1 != null && individ2 != null && !individ1.equals(individ2)) {
					...
					if (sameDirection && (Math.abs(x1 - x2) <= threshX) && (Math.abs(y1 - y2) <= threshY)) { // Da
						...
						while (groupIt.hasNext() && !existingGroup) {
							...
							// Controllo se il generico gruppo esistente contiene gi� le due persone in
							// esame
						...
						if (!existingGroup) {
							...
							// il gruppo
							// � tale dall'entrata in scena di entrambe le persone, ovvero non � nato da
							// un incontro
							boolean flag1 = false;
	 ...
	 * le aree relative alle Entry,Exit e Occluding Area; ci� viene fatto siccome
	 * in questo modo abbiamo possibilit� di conoscere facilmente se un punto (il
	 * centro del BoundingBox relativo alla perosna i-esima) � presente
